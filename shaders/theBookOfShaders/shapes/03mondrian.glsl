...
void main(){
    ...
    color += rectangle(st, vec2(0.0, 0.0)+circularMovement, vec2(0.2, 0.65));

    vec2 circularMovement2 = vec2(-sin(iGlobalTime*5.0)*0.1, cos(iGlobalTime*5.0)*0.1);
    color += rectangle(st, vec2(0.22, 0.0)+circularMovement2, vec2(0.5, 0.1));
