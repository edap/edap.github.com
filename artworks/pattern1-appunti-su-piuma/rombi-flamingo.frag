...
void main(void){
  ...
  vec3 coloreUno = vec3(0.055, 0.8, 0.812);
  vec3 coloreDue = vec3(0.965, 0.914, 0.396);
  vec3 coloreTre = vec3(0.902, 0.659, 0.141);
  vec3 coloreQuattro = vec3(0.953, 0.447, 0.035);
  vec3 coloreCinque = vec3(1., 0.267, 0.016);  
  ...
  float zoom = 1.03; // zoom solo con numeri dispari
  ...
  // rombi
  float r11 = 1.0-sdRhombus(grid2-vec2(0.5), vec2(0.5,0.5));
  ...
  vec3 colorStripes = mix(coloreTre * tex.r, coloreCinque, offset);
