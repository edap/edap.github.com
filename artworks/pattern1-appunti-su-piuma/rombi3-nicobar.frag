...
void main(void){
  ...
  vec3 coloreUno = vec3(0.965, 0.914, 0.396);
  vec3 coloreDue = vec3(0.333, 1, 0.235);
  vec3 coloreTre = vec3(0.078, 0.643, 0.8);
  vec3 coloreQuattro = vec3(0.706, .0, 0.514);
  vec3 coloreCinque = vec3(.0, .0, .0);  
  ...
  vec3 coloreStripes = mix(coloreDue * tex.r, coloreTre, offset);

  color = mix(color, coloreStripes, r11);
