...
void main(void){
  ...
  // colors
  vec3 tex = texture2D(u_tex2,gl_FragCoord.xy/ u_resolution.xy).xyz;
  ...
  vec3 dottedCol = mix(coloreQuattro* tex.b, tex, smoothstep(0.01, 0.2, dots));
  ...
  vec3 blecoloreDuestripes = mix(coloreTre * tex.r, tex, offset);
  // colors
  color = mix(color, blecoloreDuestripes, r11);
