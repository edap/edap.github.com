...
// Constants for diagonal lines pattern
const DIAGONAL_LINES_WIDTH = 1; // mm
const DIAGONAL_LINES_PADDING = 1; // mm
...
export function createDiagonalLinesPattern(x, y, width, height, fillColor, scale = 1) {
    ...
    // Get bounds from the clipPath
    const bounds = clipPath.bounds;
    const buffer = Math.max(width, height); // Large buffer to ensure full coverage
    
    // Create vertical lines that will be rotated to become diagonal
