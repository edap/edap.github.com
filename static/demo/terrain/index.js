...
function onMouseMove(event){
    ...
    if(x > 0 && x <= window.innerWidth / 2){
        mouseOffsetRotation = x.map(window.innerWidth/2, 0, 0, Math.PI);
    }else{
        mouseOffsetRotation = x.map(window.innerWidth/2, window.innerWidth, 0, -Math.PI);
    }
    //const angle = event.clientX.map(0, window.innerWidth, -Math.PI/2, Math.PI/2);
    console.log(mouseOffsetRotation);
    //camera.setRotationFromAxisAngle(new THREE.Vector3(0,1,0), angle);
...
function moveCamera() {
    ...
    camera.rotateY(mouseOffsetRotation);
}
