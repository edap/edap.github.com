...
class GameScene {
    ...
    parseGlb = (glb) => {
        ...
        const table = createTable(glb, this.settings, this.simulation, this.scene, this.tableSize, mat);
        if (!table) {
            console.error("Failed to load table model.");
            return;
        }

        const net = createNet(glb, this.settings, this.simulation, this.scene, this.tableSize, objectScaleFactor, mat);
        if (!net) {
            console.warn("Net model not found or created.");
        }

        const paddles = createPaddles(glb, this.scene, this.tableSize, this.settings, objectScaleFactor, mat);
